...
const ModernTemplate: React.FC<PortfolioProps> = ({ data }) => {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-indigo-100 selection:text-indigo-900 overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute top-[10%] left-[10%] w-72 h-72 bg-indigo-200/20 blur-[100px] rounded-full"></div>
        <div className="absolute bottom-[10%] right-[10%] w-96 h-96 bg-violet-200/20 blur-[120px] rounded-full"></div>
      </div>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 space-y-48 pb-48">
        {/* Hero Section */}
        <section className="pt-24 md:pt-40 grid lg:grid-cols-12 gap-20 items-center">
          ...
          >
            <div className="absolute -inset-10 bg-indigo-500/10 blur-[100px] rounded-full opacity-50 animate-pulse"></div>
            {about.avatar_url && (
              <div className="relative aspect-square">
                <img 
                  src={about.avatar_url} 
                  alt={about.name} 
                  className="w-full h-full rounded-[3rem] object-cover shadow-2xl border-[12px] border-white transform hover:scale-[1.02] transition-transform duration-500"
                />
                <div className="absolute -bottom-6 -right-6 w-32 h-32 bg-white rounded-3xl shadow-2xl flex items-center justify-center border border-slate-50">
                  <div className="text-center">
                    <p className="text-xs font-black text-indigo-600 uppercase tracking-widest">Top Language</p>
                    <p className="text-xl font-black text-slate-900">{github_stats.primary_language}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {featured_projects.map((project, i) => (
              <motion.div 
                ...
                className="group bg-white border border-slate-100 rounded-[2.5rem] p-10 md:p-12 hover:shadow-2xl hover:shadow-indigo-100/50 hover:border-indigo-100 transition-all relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-50/30 rounded-bl-[5rem] -mr-16 -mt-16 group-hover:scale-110 transition-transform"></div>
        ...
        {/* Tech Ecosystem */}
        <section id="stack" className="bg-slate-900 rounded-[4rem] p-12 md:p-24 text-white relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {tech_stack.map((skill, i) => (
                <motion.div 
                  ...
                  className="bg-white/5 border border-white/10 p-8 rounded-3xl flex flex-col items-center justify-center space-y-4 hover:bg-white/10 hover:border-indigo-500/50 transition-all group"
                >
        ...
        {/* Contact Section */}
        <section id="contact" className="grid lg:grid-cols-2 gap-20 items-start pt-20">
        ...
        <footer className="pt-20 pb-12 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-black uppercase tracking-[0.3em] text-slate-300">
          ...
          <div className="flex gap-12">
            ...
            <a href={contact.socials.twitter} className="hover:text-indigo-600 transition-colors">Twitter</a>
            <a href={contact.socials.linkedin} className="hover:text-indigo-600 transition-colors">LinkedIn</a>
          </div>
        </footer>
      </main>
    </div>
